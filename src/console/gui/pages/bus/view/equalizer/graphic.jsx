...
import {
    useBusEqualizerGraphic, useBusEqualizerGraphicGain,
    useBusEqualizerGraphicOptions, useBusRTA, useDevice,
...
const GraphMeter = ({ busId, frequency }) => {
...
const Graph = ({ busId, graph }) => {
    const { disabled } = useDevice();
    const {
        has, value, set, minimum, maximum,
    } = useBusEqualizerGraphicGain(busId, graph.id);
    ...
    const meter = useMemo(() => {
        ...
        return <GraphMeter busId={busId} frequency={graph.frequency} />;
...
export default ({ busId, height }) => {
    ...
    if (!has) return null;
    return <GraphOptions busId={busId} height={height} />;
};
