...
const getPoints = (chart, parameters) => {
    ...
    const {
        key, gainReduction, makeupGain, isOff,
    } = parameters;

    if ([isOff, key, gainReduction, makeupGain].includes(undefined) || isOff) return [];

    const outputGain = key + gainReduction + makeupGain;
    return [
        [key, dbMinimum],
        [key, outputGain],
...
export const createKey = (chart, parameters) => {
    ...
    // Creating the line
    keyContainer.selectAll('#compLineKey')
        .data([points])
        .enter()
        .append('path')
        .attr('id', 'compLineKey')
