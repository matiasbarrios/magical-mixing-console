// Requirements
import { drag } from 'd3';
import { getChartThresholdTextLineHeight } from '../../../../../helpers/values';
import {
...
const pointDragged = (chart, onThresholdChanged, event) => {
    ...
    thresholdContainer.selectAll('#compThresholdButton')
...
export const createOrUpdateThreshold = (chart, parameters, onThresholdChanged, label) => {
    ...
    const thresholdTextLineHeight = getChartThresholdTextLineHeight();
    const thresholdButtonY = ((height + TOP_MARGIN) / 2) - X_AXIS_HEIGHT;

    const x = xScale(threshold);

    if (thresholdCreated) {
        thresholdContainer.select('#compThreshold')
        ...
        thresholdContainer.select('#compThresholdButton')
            .attr('transform', `translate(${x}, ${thresholdButtonY})`)
            .style('cursor', parameters.disabled ? 'not-allowed' : 'default');

        // Update the threshold label
        thresholdContainer
            .select('#compThreshold text')
            .text(label)
            .attr('y', x < thresholdTextLineHeight + X_MARGIN + Y_AXIS_LEFT_WIDTH
                ? THRESHOLD_MARGIN : -thresholdTextLineHeight)
            .style('font-size', FONT.size)
            .style('fill', parameters.disabled ? COLORS.thresholdDisabled : COLORS.thresholdText);

        // Update the icon color
        thresholdContainer
            .select('#compThresholdButton path')
    ...
    // Creating the dot
    const dotGroup = thresholdContainer.selectAll('#compThresholdButton')
        .data([[threshold, threshold]])
        .enter()
        .append('g');

    dotGroup.attr('id', 'compThresholdButton')
        .attr('transform', d => `translate(${xScale(d[0])}, ${thresholdButtonY})`)
        .style('cursor', parameters.disabled ? 'not-allowed' : 'default');

    // Making it draggable
    const pointElement = thresholdContainer.select('#compThresholdButton');
